 ...
 */
   function verticalLoop(items: any[], config: any) {
    ...
    let onChange = config.onChange,
      ...
      totalHeight: number,
      gettotalHeight = () => items[length-1].offsetTop + yPercents[length-1] / 100 * heights[length-1] - startY + spaceBefore[0] + items[length-1].offsetHeight * Number(gsap.getProperty(items[length-1], "scaleY")) + (parseFloat(config.paddingBottom) || 0),
      populateheights = () => {
        let b1 = container.getBoundingClientRect(), b2;
        items.forEach((el, i) => {
          heights[i] = parseFloat(String(gsap.getProperty(el as HTMLElement, "height", "px")));
          yPercents[i] = snap(parseFloat(String(gsap.getProperty(el as HTMLElement, "y", "px"))) / heights[i] * 100 + Number(gsap.getProperty(el as HTMLElement, "yPercent")));
    ...
    tl.next = (vars: Record<string, any>) => toIndex(tl.current()+1, vars);
    tl.previous = (vars: Record<string, any>) => toIndex(tl.current()-1, vars);
    tl.times = times;
    ...
    if (config.draggable && typeof(Draggable) === "function") {
      ...
      let wrap = gsap.utils.wrap(0, 1),
        ratio: any, startProgress: any, draggable: any, dragSnap: any, lastSnap: any, initChangeY: any, wasPlaying: any,
      ...
      draggable = Draggable.create(proxy, {
        ...
        inertia: true,
        snap(value: number) {
          //note: if the user presses and releases in the middle of a throw, due to the sudden correction of proxy.y in the onPressInit(), the velocity could be very large, throwing off the snap. So sense that condition and adjust for it. We also need to set overshootTolerance to 0 to prevent the inertia from causing it to shoot past and come back
