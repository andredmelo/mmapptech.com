...
export function HomeReact3FiberViewer() {
  ...
  useGSAP(
    () => {
      const checkhomeReact3FiberViewer = setInterval(() => {
        if (document.querySelector('.homeReact3FiberViewer') && document.querySelector('.homeSection') && document.querySelector('.featuresRecordKeeperBottom') && iPhone?.scene && iPad?.scene && macBookPro?.scene) {
          clearInterval(checkhomeReact3FiberViewer);
          /* console.log("homeReact3FiberViewer is ready"); */

          const sethomeReact3FiberViewerOpacity = gsap.quickSetter(".homeReact3FiberViewer", "opacity");

          // Initial settings and positioning
          sethomeReact3FiberViewerOpacity(0);
          ...
          gsap.set(iPad.scene.position, {x: 0, y: 0, z: 0});
          ...
          sethomeReact3FiberViewerOpacity(1);


          // Pin homeReact3FiberViewer
          ...
          // iPhoneHomeAnimIn
          const iPhoneHomeAnimIn = gsap.timeline({paused:true, delay:0.125, fastScrollEnd: 3000})
              /* .set(iPhone.scene.scale, {x: 6, y: 6, z: 6})
              .set(iPhone.scene.position, {x: 0, y: 0, z: 0}, "<")
              .set(iPhone.scene.rotation, {x: 0, y: 0, z: 0}, "<") */
              .to(".homeReact3FiberViewer", {opacity:1, duration:0.1}, "<")
          ...
          const iPhoneHomeAnimOut = gsap.timeline({
            ...
            .fromTo(iPhone.scene.position, {x: 0, y: -0.6}, {x: 2, y: -3, ease:"power1.in"}, 0)
            .to(iPhone.scene.scale, {x: 8, y: 8, z: 8, ease:"linear"}, "<")
            .fromTo(iPhone.scene.rotation, {x: -0.5}, {x: -2.2, ease:"power1.out"}, "<")
            .set(iPhone.scene.rotation, {x: 0})
  ...
  return (
    <div ref={container} className="homeReact3FiberViewer absolute top-0 left-0 z-[2] h-screen w-screen overflow-visible">
      <Canvas linear>
        <CustomCamera />
